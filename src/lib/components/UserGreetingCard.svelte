	...
	<div class="flex items-center gap-3">
		{#if avatarSrc}
			<img
				src={avatarSrc}
				alt={name}
				class="size-12 rounded-full object-cover shrink-0 border-2 border-stroke"
			/>
		{:else}
			<div
				class="size-12 rounded-full shrink-0 bg-primary flex items-center justify-center text-white text-sm font-bold"
			>
				{initials}
			</div>
		{/if}
